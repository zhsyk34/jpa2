...
public class BaseDaoImpl<T> implements IBaseDao {
    ...
    public Long getCount(Query query) {
        //TODO:TEST
        CriteriaQuery criteriaQuery = query.getCriteriaQuery();
        Selection criteriaQuerySelection = criteriaQuery.getSelection();
        CriteriaBuilder builder = query.getCriteriaBuilder();
        Set<javax.persistence.criteria.Root> roots = criteriaQuery.getRoots();
        javax.persistence.criteria.Root[] array = roots.toArray(new Root[roots.size()]);
        criteriaQuery.select(builder.count(array[0]));
